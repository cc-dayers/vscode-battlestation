...
var searchGroupPickerOpen = false;
...
var bulkAssignToGroup = (items, groupName) => {
...
var renderButton = (item) => {
  ...
  if (display.showCommand) {
    ...
    if (cmdMeta.trim().toLowerCase() !== nameLower && item.command.trim().toLowerCase() !== nameLower) {
...
var renderSearch = (visibleActions) => {
  ...
  return html`
    ...
    renderView();
                    ...
                    ${searchGroupPickerOpen ? html`
                        <div class="lp-search-assign-picker" @click=${(e) => e.stopPropagation()}>
