...
var showDeleteHistoryConfirm = false;
var showAdvanced = false;
var toolbarDragSrc = null;
...
var toggleDeleteHistoryConfirm = () => {
  showDeleteHistoryConfirm = !showDeleteHistoryConfirm;
  renderView();
};
var hideDeleteHistoryConfirm = () => {
...
var configBtn = (icon, label, command, opts) => html`
  <button
    type="button"
    class=${classMap({
  "lp-config-btn": true,
...
var renderView = () => {
  ...
  render(
    html`
      ...
      onClick: () => {
        const delHistory = document.getElementById("deleteHistoryAlso")?.checked;
            ...
            <div class="lp-setting-row">
              <div class="lp-setting-label">
                ...
                <div class="lp-setting-desc">
                  ${state.customConfigPath ? html`Custom: <code class="lp-inline-code">${state.customConfigPath}/battle.json</code>` : html`Default: <code class="lp-inline-code">.vscode/battle.json</code>`}
                </div>
              </div>
        ...
        <div class="lp-setting-group">
          ...
          <label class="lp-setting-toggle">
            <input
              ...
              @change=${(e) => setState({ showCommand: e.target.checked })}
            />
          ...
          <label class="lp-setting-toggle">
            <input
              ...
              @change=${(e) => setState({ showGroup: e.target.checked })}
            />
            <span class="lp-toggle-slider"></span>
          </label>
        </div>
          ...
          <select
            ...
            @change=${(e) => setState({ hideIcon: e.target.value })}
          >
      ...
      (opt) => html`<option value=${opt.value}>${opt.label}</option>`
        ...
        <div class="lp-setting-group">
          ...
          ${(() => {
      const allIds = TOOLBAR_BTN_DEFS.map((b) => b.id);
      ...
      return orderedIds.map((id) => {
        ...
        return html`
          ...
          e.preventDefault();
                    ...
                    @drop=${(e) => {
          e.preventDefault();
          if (!toolbarDragSrc || toolbarDragSrc === id) {
            toolbarDragSrc = null;
            return;
          }
          const src = toolbarDragSrc;
          toolbarDragSrc = null;
          const newOrdered = orderedIds.filter((i) => i !== src);
          newOrdered.splice(newOrdered.indexOf(id), 0, src);
          setState({ actionToolbar: newOrdered.filter((i) => inlineIds.includes(i)) });
        }}
                    @dragend=${() => {
          toolbarDragSrc = null;
          renderView();
